...
void cpu_test_tree_creation(const int elements, const int excpectedHeight)
{
...
BOOST_AUTO_TEST_CASE(cpu_tree_with_0_height)
{
	...
	for (int size : sizes)
	{
		cpu_test_tree_creation<PAGE_SIZE>(size, 0);
	}
}

BOOST_AUTO_TEST_CASE(cpu_tree_with_1_height)
{
	const int PAGE_SIZE = 16;
	int sizes[] = {
	...
	for (int size : sizes)
	{
		cpu_test_tree_creation<PAGE_SIZE>(size, 1);
	}
}

BOOST_AUTO_TEST_CASE(cpu_tree_with_2_height)
{
	const int PAGE_SIZE = 16;
	int sizes[] = {
	...
	for (int size : sizes)
	{
		cpu_test_tree_creation<PAGE_SIZE>(size, 2);
	}
}

BOOST_AUTO_TEST_CASE(cpu_tree_with_height_greater_than_2)
{
	const int PAGE_SIZE = 16;
	int sizes[] = {
		2'000,
	...
	for (int size : sizes)
	{
		cpu_test_tree_creation<PAGE_SIZE>(size, compute_height<PAGE_SIZE>(size));
	}
}
