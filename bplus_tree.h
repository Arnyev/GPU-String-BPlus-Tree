...
protected:
	...
	void virtual create_tree(HASH* keys, int* values, int size, const char* suffixes, int suffixesLength) = 0;
public:
	 ...
	 */
	bool virtual exist_word(const char *word) = 0;
	/**
	 ...
	 * \param wordsSize Size of the 'words' array.
	 * \param beginIndexes Indexes of begins of words from the 'words' array.
	 * \param indexSize Size of the 'beginIndexes' array.
	 * \return Vector of values indicating which of provided words exists in the tree.
	 */
	std::vector<bool> virtual exist_word(const char *words, int wordsSize, int *beginIndexes, int indexSize) = 0;
